...
type PetQuery struct {
	...
	withCuriosities *CuriosityQuery
...
// QueryCuriosities chains the current query on the "curiosities" edge.
func (pq *PetQuery) QueryCuriosities() *CuriosityQuery {
	query := (&CuriosityClient{config: pq.config}).Query()
...
func (pq *PetQuery) QueryOwners() *UserQuery {
...
func (pq *PetQuery) Clone() *PetQuery {
	...
	return &PetQuery{
		...
		withCuriosities: pq.withCuriosities.Clone(),
...
// WithCuriosities tells the query-builder to eager-load the nodes that are connected to
// the "curiosities" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PetQuery) WithCuriosities(opts ...func(*CuriosityQuery)) *PetQuery {
	query := (&CuriosityClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withCuriosities = query
	return pq
}
...
func (pq *PetQuery) WithOwners(opts ...func(*UserQuery)) *PetQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withCuriosities; query != nil {
		if err := pq.loadCuriosities(ctx, query, nodes,
			func(n *Pet) { n.Edges.Curiosities = []*Curiosity{} },
			func(n *Pet, e *Curiosity) { n.Edges.Curiosities = append(n.Edges.Curiosities, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withOwners; query != nil {
		if err := pq.loadOwners(ctx, query, nodes,
			func(n *Pet) { n.Edges.Owners = []*User{} },
			func(n *Pet, e *User) { n.Edges.Owners = append(n.Edges.Owners, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadCuriosities(ctx context.Context, query *CuriosityQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Curiosity)) error {
	...
	query.withFKs = true
	query.Where(predicate.Curiosity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(pet.CuriositiesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.pet_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "pet_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (pq *PetQuery) loadOwners(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Pet)
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(pet.OwnersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(pet.OwnersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(pet.OwnersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(pet.OwnersPrimaryKey[1]))
