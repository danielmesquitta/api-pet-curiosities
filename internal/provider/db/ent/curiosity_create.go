// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"errors"
	"fmt"
	"time"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/danielmesquitta/api-pet-curiosities/internal/provider/db/ent/curiosity"
	"github.com/danielmesquitta/api-pet-curiosities/internal/provider/db/ent/pet"
	"github.com/danielmesquitta/api-pet-curiosities/internal/provider/db/ent/usercuriosity"
	"github.com/google/uuid"
)

// CuriosityCreate is the builder for creating a Curiosity entity.
type CuriosityCreate struct {
	config
	mutation *CuriosityMutation
	hooks    []Hook
}

// SetTitle sets the "title" field.
func (cc *CuriosityCreate) SetTitle(s string) *CuriosityCreate {
	cc.mutation.SetTitle(s)
	return cc
}

// SetContent sets the "content" field.
func (cc *CuriosityCreate) SetContent(s string) *CuriosityCreate {
	cc.mutation.SetContent(s)
	return cc
}

// SetCreatedAt sets the "created_at" field.
func (cc *CuriosityCreate) SetCreatedAt(t time.Time) *CuriosityCreate {
	cc.mutation.SetCreatedAt(t)
	return cc
}

// SetNillableCreatedAt sets the "created_at" field if the given value is not nil.
func (cc *CuriosityCreate) SetNillableCreatedAt(t *time.Time) *CuriosityCreate {
	if t != nil {
		cc.SetCreatedAt(*t)
	}
	return cc
}

// SetUpdatedAt sets the "updated_at" field.
func (cc *CuriosityCreate) SetUpdatedAt(t time.Time) *CuriosityCreate {
	cc.mutation.SetUpdatedAt(t)
	return cc
}

// SetNillableUpdatedAt sets the "updated_at" field if the given value is not nil.
func (cc *CuriosityCreate) SetNillableUpdatedAt(t *time.Time) *CuriosityCreate {
	if t != nil {
		cc.SetUpdatedAt(*t)
	}
	return cc
}

// SetID sets the "id" field.
func (cc *CuriosityCreate) SetID(u uuid.UUID) *CuriosityCreate {
	cc.mutation.SetID(u)
	return cc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (cc *CuriosityCreate) SetNillableID(u *uuid.UUID) *CuriosityCreate {
	if u != nil {
		cc.SetID(*u)
	}
	return cc
}

// SetPetID sets the "pet" edge to the Pet entity by ID.
func (cc *CuriosityCreate) SetPetID(id uuid.UUID) *CuriosityCreate {
	cc.mutation.SetPetID(id)
	return cc
}

// SetNillablePetID sets the "pet" edge to the Pet entity by ID if the given value is not nil.
func (cc *CuriosityCreate) SetNillablePetID(id *uuid.UUID) *CuriosityCreate {
	if id != nil {
		cc = cc.SetPetID(*id)
	}
	return cc
}

// SetPet sets the "pet" edge to the Pet entity.
func (cc *CuriosityCreate) SetPet(p *Pet) *CuriosityCreate {
	return cc.SetPetID(p.ID)
}

// AddUserCuriosityIDs adds the "user_curiosities" edge to the UserCuriosity entity by IDs.
func (cc *CuriosityCreate) AddUserCuriosityIDs(ids ...uuid.UUID) *CuriosityCreate {
	cc.mutation.AddUserCuriosityIDs(ids...)
	return cc
}

// AddUserCuriosities adds the "user_curiosities" edges to the UserCuriosity entity.
func (cc *CuriosityCreate) AddUserCuriosities(u ...*UserCuriosity) *CuriosityCreate {
	ids := make([]uuid.UUID, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return cc.AddUserCuriosityIDs(ids...)
}

// Mutation returns the CuriosityMutation object of the builder.
func (cc *CuriosityCreate) Mutation() *CuriosityMutation {
	return cc.mutation
}

// Save creates the Curiosity in the database.
func (cc *CuriosityCreate) Save(ctx context.Context) (*Curiosity, error) {
	cc.defaults()
	return withHooks(ctx, cc.sqlSave, cc.mutation, cc.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (cc *CuriosityCreate) SaveX(ctx context.Context) *Curiosity {
	v, err := cc.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (cc *CuriosityCreate) Exec(ctx context.Context) error {
	_, err := cc.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (cc *CuriosityCreate) ExecX(ctx context.Context) {
	if err := cc.Exec(ctx); err != nil {
		panic(err)
	}
}

// defaults sets the default values of the builder before save.
func (cc *CuriosityCreate) defaults() {
	if _, ok := cc.mutation.CreatedAt(); !ok {
		v := curiosity.DefaultCreatedAt()
		cc.mutation.SetCreatedAt(v)
	}
	if _, ok := cc.mutation.UpdatedAt(); !ok {
		v := curiosity.DefaultUpdatedAt()
		cc.mutation.SetUpdatedAt(v)
	}
	if _, ok := cc.mutation.ID(); !ok {
		v := curiosity.DefaultID()
		cc.mutation.SetID(v)
	}
}

// check runs all checks and user-defined validators on the builder.
func (cc *CuriosityCreate) check() error {
	if _, ok := cc.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`ent: missing required field "Curiosity.title"`)}
	}
	if v, ok := cc.mutation.Title(); ok {
		if err := curiosity.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`ent: validator failed for field "Curiosity.title": %w`, err)}
		}
	}
	if _, ok := cc.mutation.Content(); !ok {
		return &ValidationError{Name: "content", err: errors.New(`ent: missing required field "Curiosity.content"`)}
	}
	if v, ok := cc.mutation.Content(); ok {
		if err := curiosity.ContentValidator(v); err != nil {
			return &ValidationError{Name: "content", err: fmt.Errorf(`ent: validator failed for field "Curiosity.content": %w`, err)}
		}
	}
	if _, ok := cc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "Curiosity.created_at"`)}
	}
	if _, ok := cc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "Curiosity.updated_at"`)}
	}
	return nil
}

func (cc *CuriosityCreate) sqlSave(ctx context.Context) (*Curiosity, error) {
	if err := cc.check(); err != nil {
		return nil, err
	}
	_node, _spec := cc.createSpec()
	if err := sqlgraph.CreateNode(ctx, cc.driver, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	if _spec.ID.Value != nil {
		if id, ok := _spec.ID.Value.(*uuid.UUID); ok {
			_node.ID = *id
		} else if err := _node.ID.Scan(_spec.ID.Value); err != nil {
			return nil, err
		}
	}
	cc.mutation.id = &_node.ID
	cc.mutation.done = true
	return _node, nil
}

func (cc *CuriosityCreate) createSpec() (*Curiosity, *sqlgraph.CreateSpec) {
	var (
		_node = &Curiosity{config: cc.config}
		_spec = sqlgraph.NewCreateSpec(curiosity.Table, sqlgraph.NewFieldSpec(curiosity.FieldID, field.TypeUUID))
	)
	if id, ok := cc.mutation.ID(); ok {
		_node.ID = id
		_spec.ID.Value = &id
	}
	if value, ok := cc.mutation.Title(); ok {
		_spec.SetField(curiosity.FieldTitle, field.TypeString, value)
		_node.Title = value
	}
	if value, ok := cc.mutation.Content(); ok {
		_spec.SetField(curiosity.FieldContent, field.TypeString, value)
		_node.Content = value
	}
	if value, ok := cc.mutation.CreatedAt(); ok {
		_spec.SetField(curiosity.FieldCreatedAt, field.TypeTime, value)
		_node.CreatedAt = value
	}
	if value, ok := cc.mutation.UpdatedAt(); ok {
		_spec.SetField(curiosity.FieldUpdatedAt, field.TypeTime, value)
		_node.UpdatedAt = value
	}
	if nodes := cc.mutation.PetIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
			Table:   curiosity.PetTable,
			Columns: []string{curiosity.PetColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(pet.FieldID, field.TypeUUID),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_node.pet_id = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := cc.mutation.UserCuriositiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   curiosity.UserCuriositiesTable,
			Columns: []string{curiosity.UserCuriositiesColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(usercuriosity.FieldID, field.TypeUUID),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}

// CuriosityCreateBulk is the builder for creating many Curiosity entities in bulk.
type CuriosityCreateBulk struct {
	config
	err      error
	builders []*CuriosityCreate
}

// Save creates the Curiosity entities in the database.
func (ccb *CuriosityCreateBulk) Save(ctx context.Context) ([]*Curiosity, error) {
	if ccb.err != nil {
		return nil, ccb.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(ccb.builders))
	nodes := make([]*Curiosity, len(ccb.builders))
	mutators := make([]Mutator, len(ccb.builders))
	for i := range ccb.builders {
		func(i int, root context.Context) {
			builder := ccb.builders[i]
			builder.defaults()
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*CuriosityMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, ccb.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, ccb.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				mutation.id = &nodes[i].ID
				mutation.done = true
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, ccb.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (ccb *CuriosityCreateBulk) SaveX(ctx context.Context) []*Curiosity {
	v, err := ccb.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (ccb *CuriosityCreateBulk) Exec(ctx context.Context) error {
	_, err := ccb.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (ccb *CuriosityCreateBulk) ExecX(ctx context.Context) {
	if err := ccb.Exec(ctx); err != nil {
		panic(err)
	}
}
