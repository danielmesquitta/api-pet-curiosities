...
func (lq *LikeQuery) QueryCuriosity() *CuriosityQuery {
	query := (&CuriosityClient{config: lq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LikeQuery) Clone() *LikeQuery {
	...
	return &LikeQuery{
		...
		withCuriosity: lq.withCuriosity.Clone(),
...
func (lq *LikeQuery) WithCuriosity(opts ...func(*CuriosityQuery)) *LikeQuery {
	query := (&CuriosityClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withCuriosity = query
	return lq
}
...
func (lq *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	var (
		...
		loadedTypes = [2]bool{
			lq.withUser != nil,
			lq.withCuriosity != nil,
		}
	)
	if lq.withUser != nil || lq.withCuriosity != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withCuriosity; query != nil {
		if err := lq.loadCuriosity(ctx, query, nodes, nil,
			func(n *Like, e *Curiosity) { n.Edges.Curiosity = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LikeQuery) loadCuriosity(ctx context.Context, query *CuriosityQuery, nodes []*Like, init func(*Like), assign func(*Like, *Curiosity)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Like)
	for i := range nodes {
