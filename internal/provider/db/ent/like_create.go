...
func (lc *LikeCreate) SetCuriosityID(id uuid.UUID) *LikeCreate {
	lc.mutation.SetCuriosityID(id)
	return lc
}

// SetNillableCuriosityID sets the "curiosity" edge to the Curiosity entity by ID if the given value is not nil.
func (lc *LikeCreate) SetNillableCuriosityID(id *uuid.UUID) *LikeCreate {
	if id != nil {
		lc = lc.SetCuriosityID(*id)
	}
	return lc
}

// SetCuriosity sets the "curiosity" edge to the Curiosity entity.
func (lc *LikeCreate) SetCuriosity(c *Curiosity) *LikeCreate {
	return lc.SetCuriosityID(c.ID)
}
...
func (lc *LikeCreate) check() error {
	if _, ok := lc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "createdAt", err: errors.New(`ent: missing required field "Like.createdAt"`)}
	}
	return nil
}
